...
- **自动测试**：插入 U 盘 100ms 后自动向 `test/` 目录写入 `test.mp4`（94.8MB），计算写入速度，生成日志，测后删除视频
- **三列界面（1:2:1）**：
...
- `ClusterBitMap`：批量 bitmap 分配（预分配 95MB 仅需 22ms）
...
3. 应用自动挂载并测试：`test/` 目录写入 `test.mp4` → 测速 → 写日志 → 删视频
...
- `test.mp4`（94.8MB，截取自测试视频）打包在 `app/src/main/assets/test_files/`
- 首次启动自动复制到应用内部存储（`files/test_files/`），后续直接使用
- 替换测试文件：将新文件放入 assets 目录并重新打包，或替换内部存储中的文件

## 注意事项

- 测试文件 94.8MB 接近 GitHub 100MB 限制，使用 **Git LFS** 管理（克隆需安装 git-lfs）
- 版本号按打包时间生成：`versionName=1.0.yyyyMMdd.HHmm`、`versionCode=yyyyMMdd`，界面"U 盘列表"标题右侧显示
